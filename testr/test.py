...
# auth = requests.auth.HTTPBasicAuth('melon', 'melon')
...
cookie = {'PHPSESSID': '8b3laq9gmbdj8qf82qbhadc2ia'} 

session = requests.Session()
for i in range(len(result_ids)):
    response = session.post(url, auth=requests.auth.HTTPBasicAuth('melon', 'melon'), data={"email": result_ids[i], "password": result_passwords[i]},cookies=cookie)
